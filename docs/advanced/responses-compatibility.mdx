...
<Note>
  On Anthropic, `text.format` is served by the provider's native structured
  outputs (`json_schema`) or by a system-prompt instruction (`json_object`) —
  see [Anthropic](/providers/anthropic#structured-output).
  `text.verbosity` has no Anthropic equivalent and is dropped with a warning.
</Note>
