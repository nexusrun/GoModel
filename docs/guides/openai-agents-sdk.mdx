...
- On Anthropic, `text.format` is served by Claude's native structured outputs
  (`json_schema`) or a system-prompt instruction (`json_object`), and
  `text.verbosity` is dropped with a warning — Anthropic has no equivalent.
