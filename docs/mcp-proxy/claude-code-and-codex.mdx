...
| Last error says `no MCP endpoint at that path` (older releases: `session not found` during the initial connection) | The server's `url` includes its MCP endpoint path, for example `/http` for mcp-devtools or `/v2/mcp` for Firecrawl |
| Last error mentions `HTTP 405 Method Not Allowed` | The server's `transport` matches the endpoint: `http` for streamable HTTP, `sse` only for legacy SSE servers |
